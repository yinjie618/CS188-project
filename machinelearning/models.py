...
class PerceptronModel(Module):
    def __init__(self, dimensions):
        ...
        "*** YOUR CODE HERE ***"
        weightVector = ones(1,dimensions)
        self.w = Parameter(weightVector)
...
class RegressionModel(Module):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__()
        self.w1 = Parameter(randn(1, 100)*0.01)
        self.b1 = Parameter(zeros(1, 100))
        self.w2 = Parameter(randn(100, 50)*0.01)
        self.b2 = Parameter(zeros(1, 50))
        self.w3 = Parameter(randn(50, 1)*0.01)
        self.b3 = Parameter(zeros(1, 1))
...
class DigitClassificationModel(Module):
    ...
    def __init__(self):
        ...
        output_size = 10
        "*** YOUR CODE HERE ***"
        self.w1 = Parameter(randn(input_size, 100)*0.01)
        self.b1 = Parameter(zeros(1, 100))
        self.w2 = Parameter(randn(100, 50)*0.01)
        self.b2 = Parameter(zeros(1, 50))
        self.w3 = Parameter(randn(50, output_size)*0.01)
...
class LanguageIDModel(Module):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        # Initialize your model parameters here
        self.Wx = Parameter(randn(self.num_chars, 200) * 0.01)
        self.Wh = Parameter(randn(200, 200) * 0.01)
        self.bh = Parameter(zeros(1, 200))
        self.W_out = Parameter(randn(200, len(self.languages)) * 0.01)
        self.b_out = Parameter(zeros(1, len(self.languages)))
    ...
    def forward(self, xs):
        ...
        """
        "*** YOUR CODE HERE ***"
        h = zeros(xs[0].shape[0],self.Wh.shape[0])
        for xI in xs:
            h = relu(xI @ self.Wx + h @ self.Wh + self.bh)
...
def Convolve(input: tensor, weight: tensor):
    ...
    outputH = input_tensor_dimensions[0] - weight_dimensions[0] + 1
    outputW = input_tensor_dimensions[1] - weight_dimensions[1] + 1
    Output_Tensor = zeros((outputH, outputW))
    for i in range(outputH):
        for j in range(outputW):
            patch = input[i:i+weight_dimensions[0], j:j+weight_dimensions[1]]
...
class DigitConvolutionalModel(Module):
    ...
    def __init__(self):
        ...
        self.w1 = Parameter(randn(conv_output_size, 128) * 0.01)
        self.b1 = Parameter(zeros(1, 128))
        self.w2 = Parameter(randn(128, 64) * 0.01)
        self.b2 = Parameter(zeros(1, 64))
        self.w3 = Parameter(randn(64, output_size) * 0.01)
        self.b3 = Parameter(zeros(1, output_size))
...
class Attention(Module):
    ...
    def forward(self, input):
        ...
        kReverse = movedim(K, 1, 2)
        qKT = Q @ kReverse
        M = qKT / (self.layer_size ** 0.5)
        M = M.masked_fill(self.mask[:,:,:T,:T] == 0, float('-inf'))[0]
        output = softmax(M, dim = -1) @ V
        return output
