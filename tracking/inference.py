...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict["Pacman"] = map
    variableDomainsDict["Ghost0"] = map
    variableDomainsDict["Ghost1"] = map

    noisyDistances = list(range(0, (X_RANGE - 1) + (Y_RANGE - 1) + MAX_NOISE + 1))
    variableDomainsDict["Observation0"] = noisyDistances
    variableDomainsDict["Observation1"] = noisyDistances
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        allValues = set(bayesNet.variablesSet())
        queryVariables = set(queryVariables)
        evidenceValues = set(evidenceDict.keys())

        hiddenValues = allValues - queryVariables - evidenceValues

        for value in eliminationOrder:
            if value not in hiddenValues:
                continue

            factors, joinedFactor = joinFactorsByVariable(factors, value)
            
            if len(joinedFactor.unconditionedVariables()) > 1:
                eliminated = eliminate(joinedFactor,value)
                factors.append(eliminated)

        finalFactors = joinFactors(factors)
        result = normalize(finalFactors)
        return result
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPos in self.allPositions:
            newBeliefs[ghostPos] = self.getObservationProb(observation, gameState.getPacmanPosition(), ghostPos, self.getJailPosition()) * self.getBeliefDistribution()[ghostPos]
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        newBeliefs = DiscreteDistribution()
        oldPosDist = self.allPositions

        for oldPos in oldPosDist:
            newPosDist = self.getPositionDistribution(gameState, oldPos).items()
            oldBelief = self.beliefs[oldPos]
            for newPos, prob in newPosDist:
                newBeliefs[newPos] = newBeliefs[newPos] + oldBelief * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for posIndex in range(self.numParticles):
            index = posIndex % len(self.legalPositions)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in particles:
            prob = self.getObservationProb(noisyDistance, pacmanPos, p, jailPos)
            weights[p] = weights[p] + prob
